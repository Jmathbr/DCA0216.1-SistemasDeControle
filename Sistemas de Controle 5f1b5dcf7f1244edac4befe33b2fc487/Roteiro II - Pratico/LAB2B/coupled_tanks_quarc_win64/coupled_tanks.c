...
void coupled_tanks_output(void)
{
  ...
  /* S-Function Block: coupled_tanks/PLANTA/HIL Read Analog (hil_read_analog_block) */
  {
    ...
    rtb_Saturation = coupled_tanks_DW.HILReadAnalog_Buffer[0];
    rtb_HILReadAnalog_o2 = coupled_tanks_DW.HILReadAnalog_Buffer[1];
  }

  /* Gain: '<S2>/Volts to cm Tank 1 ' */
  rtb_VoltstocmTank1 = coupled_tanks_P.VoltstocmTank1_Gain * rtb_Saturation;

  /* Constant: '<Root>/Constant' */
  coupled_tanks_B.Constant = coupled_tanks_P.Constant_Value;

  /* Saturate: '<Root>/Saturation' */
  if (coupled_tanks_B.Constant > coupled_tanks_P.Saturation_UpperSat) {
    rtb_Saturation = coupled_tanks_P.Saturation_UpperSat;
  } else if (coupled_tanks_B.Constant < coupled_tanks_P.Saturation_LowerSat) {
    rtb_Saturation = coupled_tanks_P.Saturation_LowerSat;
  } else {
    rtb_Saturation = coupled_tanks_B.Constant;
  }
  ...
  /* S-Function Block: coupled_tanks/PLANTA/HIL Write Analog (hil_write_analog_block) */
  {
    ...
    result = hil_write_analog(coupled_tanks_DW.HILInitialize_Card,
      &coupled_tanks_P.HILWriteAnalog_channels, 1, &rtb_Saturation);
    if (result < 0) {
      msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
        (_rt_error_message));
      rtmSetErrorStatus(coupled_tanks_M, _rt_error_message);
  ...
  /* Clock: '<Root>/Clock' */
  coupled_tanks_B.Clock = coupled_tanks_M->Timing.t[0];

  /* Stop: '<Root>/Stop Simulation' incorporates:
   *  Constant: '<S1>/Constant'
   *  RelationalOperator: '<S1>/Compare'
   */
  if (rtb_VoltstocmTank1 >= coupled_tanks_P.CompareToConstant_const) {
  ...
  coupled_tanks_B.TmpSignalConversionAtToWorkspac[0] = coupled_tanks_B.Clock;
  coupled_tanks_B.TmpSignalConversionAtToWorkspac[1] = coupled_tanks_B.Constant;
}
...
RT_MODEL_coupled_tanks_T *coupled_tanks(void)
{
  ...
  rtsiSetSolverName(&coupled_tanks_M->solverInfo,"FixedStepDiscrete");
  ...
  /* External mode info */
  coupled_tanks_M->Sizes.checksums[0] = (3082748089U);
  coupled_tanks_M->Sizes.checksums[1] = (3775557808U);
  coupled_tanks_M->Sizes.checksums[2] = (4289032521U);
  coupled_tanks_M->Sizes.checksums[3] = (2611712617U);
  ...
  {
    coupled_tanks_B.Constant = 0.0;
    coupled_tanks_B.Clock = 0.0;
    coupled_tanks_B.TmpSignalConversionAtToWorkspac[0] = 0.0;
    coupled_tanks_B.TmpSignalConversionAtToWorkspac[1] = 0.0;
  }
  ...
  coupled_tanks_M->Sizes.numBlocks = (13);/* Number of blocks */
  coupled_tanks_M->Sizes.numBlockIO = (3);/* Number of block outputs */
  coupled_tanks_M->Sizes.numBlockPrms = (104);/* Sum of parameter "widths" */
